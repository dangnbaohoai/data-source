<template>
  <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
    <div class="w-full overflow-x-auto">
      <h1 class="text-3xl px-4 font-bold">Reported comment</h1>
      ...
      <table class="w-full">
        ...
        <tbody class="bg-white">
          <tr class="text-gray-700" v-for="data in datas" :key="data.reportId">
            ...
            <td class="px-2 py-4 text-sm border flex items-center h-full">
              <router-link
                class="hover:bg-gray-100 border- px-1"
                title="go to the post"
                :to="{
                  path: '/Forum/' + 15 + '/post/' + data.comment.postId,
                }"
                target="_blank"
              >
                <svg
          ...
          <tr
            ...
            v-if="datas == [] || datas == null"
          >
            <td colspan="6" class="text-center text-2xl">
...
export default {
  ...
  methods: {
    ...
    async deleteReport(id) {
      const res = await DeleteCMTReport(id.reportId, this.forumId);
    ...
    async cancelPostReport(id) {
      const res = await CancelCMTReport(id.reportId, this.forumId);
    ...
    async getReportData() {
      this.datas = await getCMTReport(Number.parseInt(this.forumId), this.page);
    ...
    async kickUser(data) {
      ...
      await this.$confirm.require({
        ...
        accept: async () => {
          ...
          const res = await (
            await deleteMemberOutOfForum(this.forumId, data.comment.userId)
          ).data;
